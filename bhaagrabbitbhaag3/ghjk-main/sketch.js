...
//  rabbit_collided= loadAnimation("assets/")
  ...
  bgsound = loadSound("assets/rabbit game sound.wav");
}
function setup() {
...
//  rabbit.addAnimation("collided",rabbit_collided);
  ...
  gameOver = createSprite(width/2 , height/2-30,200,20);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyDown("space") && rabbit.y >= height-150) {
      rabbit.velocityY = -22;
    }
    bgsound.play();
...
function reset(){
  ...
  rabbit.changeAnimation("running",rabbitImage);
